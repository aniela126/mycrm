...
class TaskCompl extends Common
{
	...
	{
		$link = "taskscompl";
		$tableName = (isset($_GET['gn']) && $_GET['gn'] !== '') ? $_GET['gn'] : 'taskscompl';
		include 'menu.php';
	...
	protected function initiateEditor()
	{
		...
		$this->Editor->setConfig('tableTitle','<div class="text-center"><h1>Completed Tasks</h1></div>');
		//$this->Editor->setConfig('addRowTitle','Add new task');
		$this->Editor->setConfig('editRowTitle','Edit completed task');
		...
		$this->Editor->setConfig('sqlFilters',"DATE_END IS NOT NULL"); 
